...
class X0PredLatentLMDBDataset(Dataset):
    """Dataset of Stage 3 x0-pred LR inputs and clean LR/HR Wan latent targets."""
    ...
    def __init__(
        ...
        if not self.shards:
            raise FileNotFoundError(f"No Stage 3 x0-pred LMDB shards found under {self.data_dir}")
        ...
        if not self.index:
            raise FileNotFoundError(f"No Stage 3 x0-pred samples found under {self.data_dir}")
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        x0_pred_lr = _read_array(env, "x0_pred_lr", row_id, np.float16, tuple(meta["x0_pred_lr_shape"]))
        ...
        x0_pred_lr_tensor = torch.from_numpy(x0_pred_lr.astype(np.float32, copy=False))
        ...
        if x0_pred_lr_tensor.shape != z0_lr_tensor.shape:
            raise ValueError(f"x0_pred/z0_lr shape mismatch at shard {self.shards[shard_id]} row {row_id}")
        ...
        return {
            "x0_pred_lr": x0_pred_lr_tensor.to(self.dtype),
