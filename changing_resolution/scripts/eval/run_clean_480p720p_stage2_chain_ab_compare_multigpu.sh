...
LOG_DIR="${LOG_DIR:-${PROJECT_ROOT}/logs/changing_resolution_stage2_chain_ab_compare}"
...
for rank in "${!GPUS[@]}"; do
  ...
  (
    ...
    LIMIT="${count}" \
    bash changing_resolution/scripts/eval/run_clean_480p720p_stage2_chain_ab_compare.sh
  ) >"${log_path}" 2>&1 &
  pids+=("$!")
  offset=$((offset + count))
done
...
if (( failed != 0 )); then
  echo "Stage 2 chain A/B compare failed. Check logs under: ${LOG_DIR}" >&2
  exit 1
fi

echo "Stage 2 chain A/B compare ready: ${OUT_DIR}/compare"
