...
LOG_DIR="${LOG_DIR:-${PROJECT_ROOT}/logs/changing_resolution_stage3_change_step_sweep}"
...
echo "Stage 3 change-step sweep:"
...
if [[ "${STEP_START}" == "10" && "${STEP_END}" == "50" && "${STEP_STRIDE}" == "1" && -z "${CHANGE_STEPS}" && "${TOTAL_PROMPTS}" == "4" ]]; then
  echo "Note: steps 10..50 inclusive gives 41 steps, so 4 prompts produce 164 panels." >&2
fi
...
if (( failed != 0 )); then
  echo "Stage 3 change-step sweep failed. Check logs under: ${LOG_DIR}" >&2
  exit 1
fi

echo "Stage 3 change-step sweep ready: ${OUT_DIR}/compare"
