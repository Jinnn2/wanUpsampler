...
def main() -> None:
    ...
    hr_video = read_video_frames(args.video_path, max_frames=args.max_frames)
    hr_video = resize_video(hr_video, tuple(args.hr_size), mode="bicubic").clamp(0, 1)
    lr_video = resize_video(hr_video, tuple(args.lr_size), mode=args.lr_resize_mode).clamp(0, 1)
    ...
    with torch.no_grad(), torch.autocast(device_type=device.type, dtype=autocast_dtype, enabled=use_autocast):
        pred_z0_hr = model(z0_lr, output_size=args.hr_latent_size)

    trained_video = vae.decode(pred_z0_hr.cpu())[0]

    out_path = Path(args.save_result_path)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    write_video(out_path, trained_video, fps=args.output_fps)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--hr_latent_size", type=int, nargs=2, default=[90, 156], metavar=("H", "W"))
