...
LAMBDA_TAG="${UTILITY_LAMBDA//./p}"
PRIOR_OUT="${PRIOR_OUT:-$QUALITY_DIR/utility_prior_${FEATURES}_${ACTION_SET}_lambda_${LAMBDA_TAG}}"
...
DRIVER="$PROJECT_ROOT/UNIV_adaptor/scripts/router/train_phase2_utility_prior.py"
ARGS=(--quality-dir "$QUALITY_DIR" --out-dir "$PRIOR_OUT" --features "$FEATURES"
      --action-set "$ACTION_SET" --utility-lambda "$UTILITY_LAMBDA"
