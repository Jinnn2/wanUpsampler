...
DRIVER="${PROJECT_ROOT}/UNIV_adaptor/scripts/router/train_sparse_prompt_state_router.py"
...
check_scores() {
  require_common
  "${PYTHON}" "${DRIVER}" check --scored-dir "${SCORED_DIR}"
}
...
train_router() {
  ...
  local out_dir="${OUT_DIR:-${SCORED_DIR}/prompt_state_${prompt_features}_lambda_${UTILITY_LAMBDA//./p}}"
  local -a args=(
    "${DRIVER}" train
    --scored-dir "${SCORED_DIR}"
    --state-dir "${STATE_DIR}"
...
echo "State features: ${STATE_DIR}/state_manifest.json"
