...
SOURCE_ROOT="${SOURCE_ROOT:-${PROJECT_ROOT}/outputs/univ_targeted_st_contrast_v1}"
PROTOCOL="${PROTOCOL:-${PROJECT_ROOT}/UNIV_adaptor/configs/univ_targeted_st_temporal_calibration_v2.json}"
TEMPLATE_CONFIG="${TEMPLATE_CONFIG:-${PROJECT_ROOT}/UNIV_adaptor/configs/wan21_t2v_univ_rgb_720p.example.json}"
OUT_ROOT="${OUT_ROOT:-${PROJECT_ROOT}/outputs/univ_targeted_st_temporal_calibration_v2}"
MANIFEST="${OUT_ROOT}/generation_manifest.json"
DATASET="${OUT_ROOT}/targeted_st_dataset.json"
DRIVER="${PROJECT_ROOT}/UNIV_adaptor/scripts/data/run_targeted_st_temporal_calibration.py"
SCORER="${PROJECT_ROOT}/UNIV_adaptor/scripts/data/score_targeted_st_contrast.py"
GPU_IDS="${GPU_IDS:-0,1,2,3,4,5,6,7}"
JOB_CHUNK_SIZE="${JOB_CHUNK_SIZE:-2}"
...
require_inputs() {
  ...
  [[ -f "${SOURCE_ROOT}/targeted_st_dataset.json" ]] || { echo "Source finalized dataset not found: ${SOURCE_ROOT}" >&2; exit 1; }
  for path in "${PROTOCOL}" "${TEMPLATE_CONFIG}" "${DRIVER}" "${SCORER}"; do
...
print_plan() {
  ...
  echo "Targeted S/T temporal calibration"
  echo "  source spatial videos reused: 24"
  echo "  temporal rt=0.36 videos generated: 24"
  echo "  scored paired videos: 48"
...
generate_parallel() {
  ...
  validate_gpus
  [[ -f "${MANIFEST}" ]] || { echo "Manifest not found: ${MANIFEST}" >&2; exit 1; }
  mkdir -p "${OUT_ROOT}/logs/8gpu_temporal_calibration"
  local lock="${OUT_ROOT}/.targeted_st_temporal_calibration.lock"
  ...
  for slot in 0 1 2 3 4 5 6 7; do
    ...
    local log="${OUT_ROOT}/logs/8gpu_temporal_calibration/gpu_${gpu}.log"
    (
