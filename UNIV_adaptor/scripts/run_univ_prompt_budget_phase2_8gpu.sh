...
PROTOCOL="${PROTOCOL:-${PROJECT_ROOT}/UNIV_adaptor/configs/univ_prompt_budget_phase2.json}"
PROMPTS_FILE="${PROMPTS_FILE:-${PROJECT_ROOT}/prompts/univ_controller_pilot_500.txt}"
PROMPT_LIMIT="${PROMPT_LIMIT:-161}"
TEMPLATE_CONFIG="${TEMPLATE_CONFIG:-${PROJECT_ROOT}/UNIV_adaptor/configs/wan21_t2v_univ_rgb_720p.example.json}"
OUT_ROOT="${OUT_ROOT:-${PROJECT_ROOT}/outputs/univ_prompt_budget_phase2_v1}"
MANIFEST="${OUT_ROOT}/generation_manifest.json"
DRIVER="${PROJECT_ROOT}/UNIV_adaptor/scripts/data/run_prompt_budget_generation.py"
GPU_IDS="${GPU_IDS:-0,1,2,3,4,5,6,7}"
SPLITS="${SPLITS:-train,validation}"
JOB_CHUNK_SIZE="${JOB_CHUNK_SIZE:-100}"
...
[[ "${ALLOW_PILOT_PRESETS}" == "0" || "${ALLOW_PILOT_PRESETS}" == "1" ]] || {
  echo "ALLOW_PILOT_PRESETS must be 0 or 1." >&2
  exit 2
}
  ...
  for slot in 0 1 2 3 4 5 6 7; do
    ...
    (
      while IFS= read -r job_id; do
        ...
        [[ "${ALLOW_PILOT_PRESETS}" == "1" ]] && args+=(--allow-pilot-presets)
...
case "${MODE}" in
  check)
    require_inputs
    validate_gpus
    "${WAN_PYTHON}" "${PROJECT_ROOT}/UNIV_adaptor/scripts/data/plan_univ_sparse_dataset.py" check-protocol --protocol "${PROTOCOL}"
...
for name in ("wan2.1_univ_native", "wan2.1_univ_pipeline"):
    if name not in RUNNER_REGISTER:
        raise SystemExit(f"runner registration missing: {name}")
