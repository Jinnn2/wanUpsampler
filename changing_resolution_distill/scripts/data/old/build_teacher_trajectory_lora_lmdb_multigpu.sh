...
USER_CR_DISTILL_TEACHER_TRAJ_LMDB_DIR="${CR_DISTILL_TEACHER_TRAJ_LMDB_DIR+x}"
...
if [[ -n "${USER_CR_DISTILL_TEACHER_TRAJ_LMDB_DIR}" ]]; then
  ...
  LMDB_ROOT="${PROJECT_ROOT}/data/changing_resolution_distill/lmdb_teacher_trajectory_lora_14b_cfgdistill_5k_step3"
fi
PARTS_DIR="${LMDB_ROOT}/_parts"
LOG_DIR="${LOG_DIR:-${PROJECT_ROOT}/logs/changing_resolution_distill_teacher_trajectory_lora_multigpu}"
...
for path in "${PROMPTS_FILE}" "${LIGHTX2V_REPO:-/mnt/afs_2/houze/LightX2V}" "${MODEL_ROOT}" "${CR_DISTILL_DIT_CKPT}" "${CONFIG_JSON}"; do
  if [[ ! -e "${path}" ]]; then
...
echo "Multi-GPU teacher-only LoRA LMDB build"
...
for rank in "${!GPUS[@]}"; do
  ...
  (
    ...
    CR_DISTILL_TEACHER_TRAJ_LMDB_DIR="${part_lmdb}" \
    ...
    MAX_SAMPLES="${count}" \
    OVERWRITE="${OVERWRITE}" \
    bash changing_resolution_distill/scripts/data/build_teacher_trajectory_lora_lmdb.sh
  ) >"${log_path}" 2>&1 &
...
echo "Merged teacher-only LoRA LMDB ready: ${LMDB_ROOT}"
...
python "${PROJECT_ROOT}/changing_resolution_distill/scripts/data/check_teacher_trajectory_lora_lmdb.py" "${LMDB_ROOT}" --samples 3
