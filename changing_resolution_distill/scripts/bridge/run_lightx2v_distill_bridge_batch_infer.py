...
from lightx2v.common.ops import *  # noqa: E402,F403
import changing_resolution_distill.lightx2v_distill_bridge  # noqa: E402,F401
from lightx2v.utils.input_info import init_empty_input_info, update_input_info_from_dict  # noqa: E402
from lightx2v.utils.profiler import ProfilingContext4DebugL1  # noqa: E402
from lightx2v.utils.registry_factory import RUNNER_REGISTER  # noqa: E402
from lightx2v.utils.set_config import print_config, set_config, set_parallel_config  # noqa: E402
from lightx2v.utils.utils import seed_all, validate_config_paths  # noqa: E402
from lightx2v_platform.registry_factory import PLATFORM_DEVICE_REGISTER  # noqa: E402
...
def main() -> None:
    ...
    if not jobs:
        logger.info(
            f"[batch] all {len(prompts)} output(s) already exist; skip model initialization"
        )
        cleanup_distributed()
        return
    ...
    with ProfilingContext4DebugL1("Batch Total Cost"):
        ...
        for job_index, (index, prompt, seed, output_path) in enumerate(jobs):
            ...
            logger.info(
                f"[batch] {args.name_prefix} index={index:02d} seed={seed} output={output_path}"
            )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--timing-jsonl",
        default="",
        help="Optional raw timing output for initialization, full pipeline, and run_segment.",
    )
    parser.add_argument(
        "--timing-warmup",
        type=int,
