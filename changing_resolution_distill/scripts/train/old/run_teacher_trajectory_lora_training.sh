...
check_env() {
  if [[ ! -f "${CR_DISTILL_TEACHER_TRAJ_CONFIG}" ]]; then
    echo "Config not found: ${CR_DISTILL_TEACHER_TRAJ_CONFIG}" >&2
    exit 1
  fi
  if [[ ! -d "${CR_DISTILL_TEACHER_TRAJ_LMDB_DIR}" ]] || [[ -z "$(find "${CR_DISTILL_TEACHER_TRAJ_LMDB_DIR}" -type f -name 'data.mdb' -print -quit 2>/dev/null)" ]]; then
    echo "Teacher trajectory LMDB not found: ${CR_DISTILL_TEACHER_TRAJ_LMDB_DIR}" >&2
    exit 1
  fi
...
train_lora() {
  check_env
  mkdir -p "${CR_DISTILL_TEACHER_TRAJ_OUT_DIR}"

  args=(
    --config "${CR_DISTILL_TEACHER_TRAJ_CONFIG}"
    --data_dir "${CR_DISTILL_TEACHER_TRAJ_LMDB_DIR}"
    --out_dir "${CR_DISTILL_TEACHER_TRAJ_OUT_DIR}"
  ...
  echo "  out_dir     : ${CR_DISTILL_TEACHER_TRAJ_OUT_DIR}"
...
case "${MODE}" in
  check)
    check_env
    echo "Teacher-only LoRA training preflight passed."
    ;;
  train)
    train_lora
    ;;
  smoke)
    ...
    ;;
  *)
    echo "Usage: bash changing_resolution_distill/scripts/train/run_teacher_trajectory_lora_training.sh [check|smoke|train]" >&2
    exit 2
    ;;
esac
