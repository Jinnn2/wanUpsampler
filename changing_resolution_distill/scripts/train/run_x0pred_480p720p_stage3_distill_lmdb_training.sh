...
check_lmdb() {
  if [[ ! -d "${LMDB_DIR}" ]] || [[ -z "$(find "${LMDB_DIR}" -type f -name 'data.mdb' -print -quit 2>/dev/null)" ]]; then
    echo "No Stage 3 distill x0-pred LMDB shards found under: ${LMDB_DIR}" >&2
    echo "Build it first with:" >&2
    echo "  HANDOFF_STEP=${HANDOFF_STEP} bash changing_resolution_distill/scripts/data/build_x0pred_480p720p_stage3_distill_lmdb.sh" >&2
    exit 1
  fi
}
...
if tuple(y.shape) != (1, 16, 2, 6, 9):
    raise SystemExit(f"Unexpected Stage 3 distill smoke-test output shape: {tuple(y.shape)}")
print(f"Stage 3 distill model/import smoke test passed: shape={tuple(y.shape)} dataset={X0PredLatentLMDBDataset.__name__}")
PY
}

train_stage3_distill() {
  check_lmdb
  check_python_deps
  mkdir -p "${OUT_DIR}"
...
case "${MODE}" in
  check)
    ...
    check_python_deps
    check_stage3_import
    echo "Stage 3 distill LMDB training preflight passed: ${LMDB_DIR} (handoff_step=${HANDOFF_STEP})"
    ;;
  train)
    train_stage3_distill
    ;;
  *)
    echo "Usage: bash changing_resolution_distill/scripts/train/run_x0pred_480p720p_stage3_distill_lmdb_training.sh [check|train]" >&2
    exit 2
    ;;
esac
