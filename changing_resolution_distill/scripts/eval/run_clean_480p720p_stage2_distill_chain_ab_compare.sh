...
CHECKPOINT="${CR_DISTILL_STAGE2_CHAIN_COMPARE_CKPT:-${CR_DISTILL_STAGE2_OUT_DIR:-${PROJECT_ROOT}/outputs/changing_resolution_distill_clean_480p720p_stage2_${CR_DISTILL_STAGE2_TAG}_lmdb}/latest.pt}"
...
OUT_DIR="${CR_DISTILL_STAGE2_CHAIN_COMPARE_DIR:-${PROJECT_ROOT}/outputs/changing_resolution_distill_chain_ab_stage2}"

LIMIT="${LIMIT:-8}"
PROMPT_OFFSET="${PROMPT_OFFSET:-0}"
START_SEED="${START_SEED:-9600}"
...
mkdir -p "${OUT_DIR}"/{configs,interp720,stage2_720,compare}
...
export PROJECT_ROOT CHECKPOINT TRAIN_CONFIG RATE DIT_CKPT DENOISING_STEP_LIST INFER_STEPS NUM_FRAMES GUIDE_SCALE SAMPLE_SHIFT BRIDGE_USE_EMA HR_H HR_W RENOISE_MODE CHANGE_STEP STAGE2_RESIDUAL_SKIP

index=0
for prompt in "${prompts[@]}"; do
  global_index=$((PROMPT_OFFSET + index))
  sample_id="$(printf "%03d_seed%d" "${global_index}" "$((START_SEED + global_index))")"
  seed=$((START_SEED + global_index))

  cfg_interp="${OUT_DIR}/configs/${sample_id}_interp720_step${CHANGE_STEP}.json"
  cfg_stage2="${OUT_DIR}/configs/${sample_id}_stage2_720_step${CHANGE_STEP}.json"
  write_config "${cfg_interp}" "interp"
  write_config "${cfg_stage2}" "stage2"

  interp720="${OUT_DIR}/interp720/${sample_id}_interp720_step${CHANGE_STEP}.mp4"
  stage2_720="${OUT_DIR}/stage2_720/${sample_id}_stage2_720_step${CHANGE_STEP}.mp4"
  p_interp="${OUT_DIR}/compare/${sample_id}_panel_interp720_step${CHANGE_STEP}.mp4"
  p_stage2="${OUT_DIR}/compare/${sample_id}_panel_stage2_720_step${CHANGE_STEP}.mp4"
  compare="${OUT_DIR}/compare/${sample_id}_distill_stage2_chain_ab_compare.mp4"
  ...
  run_infer "wan2.1_distill_clean_resizer_bridge" "${cfg_stage2}" "${prompt}" "${seed}" "${stage2_720}"

  make_labeled_panel "${interp720}" "${p_interp}" "distill interp step ${CHANGE_STEP}"
  make_labeled_panel "${stage2_720}" "${p_stage2}" "distill stage2 step ${CHANGE_STEP}"
  ffmpeg -hide_banner -loglevel error -y \
    -i "${p_interp}" -i "${p_stage2}" \
...
echo "Distill Stage 2 chain A/B comparison videos ready: ${OUT_DIR}/compare"
