...
RENOISE_MODE="${RENOISE_MODE:-resize_flow}"
...
for step in "${model_steps[@]}"; do
  ...
  if [[ -z "${checkpoint}" ]]; then
    checkpoint="${PROJECT_ROOT}/outputs/changing_resolution_distill_x0pred_480p720p_stage3_${CR_DISTILL_STAGE3_TAG}_step${step}_lmdb/latest.pt"
  fi
