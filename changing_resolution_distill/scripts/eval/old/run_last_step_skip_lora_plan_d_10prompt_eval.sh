...
# 10-prompt eval for Plan D last-step-skip LoRA.
#
# Model weights are loaded once per case:
#   1) original Wan distill, 3 denoise steps
#   2) original Wan distill, 3 denoise steps with LoRA enabled only on step 3
#   3) original Wan distill, 4 denoise steps
#
# Usage:
#   bash changing_resolution_distill/scripts/eval/run_last_step_skip_lora_plan_d_10prompt_eval.sh
#
# Useful overrides:
...
#   OUT_ROOT=outputs/changing_resolution_distill_last_step_skip_lora_plan_d_10prompt_eval
...
OUT_ROOT="${OUT_ROOT:-${PROJECT_ROOT}/outputs/changing_resolution_distill_last_step_skip_lora_plan_d_10prompt_eval}"
...
run_case_batch "01_lora3_step3_only" "wan2.1_distill_last_step_lora" "3" "1000,750,500" "1"
...
if command -v ffmpeg >/dev/null 2>&1; then
  for ((i = 0; i < LIMIT; i++)); do
    ...
    v1="${OUT_ROOT}/videos/01_lora3_step3_only/01_lora3_step3_only_${idx}_seed${item_seed}.mp4"
    v2="${OUT_ROOT}/videos/02_original4/02_original4_${idx}_seed${item_seed}.mp4"
    stacked="${OUT_ROOT}/compare/plan_d_eval_${idx}_seed${item_seed}_original3_lora3_original4.mp4"
    if [[ -f "${v0}" && -f "${v1}" && -f "${v2}" ]]; then
      echo "[10prompt] creating hstack index=${idx}: ${stacked}"
      ffmpeg -y \
        -i "${v0}" \
        -i "${v1}" \
        -i "${v2}" \
      ...
      echo "[10prompt] skip hstack index=${idx}; missing one or more videos" >&2
    fi
  done
else
  echo "[10prompt] ffmpeg not found; skip hstack"
fi
