...
SESSION_NAME="${SESSION_NAME:-oracle_dataset_1500_2gpu}"
GPU_IDS="${GPU_IDS:-0,1}"
...
LOG_FILE="${PROJECT_ROOT}/logs/oracle_dataset_1500_2gpu/${SESSION_NAME}.log"
...
printf -v RUN_COMMAND \
  ...
  "${SCRIPT_DIR}/build_oracle_dataset_1500_2gpu.sh" "${LOG_FILE}"

tmux new-session -d -s "${SESSION_NAME}" "${RUN_COMMAND}"

echo "Started successfully in tmux."
