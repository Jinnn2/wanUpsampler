...
echo "  Seeds        : ${SEEDS}"
echo "  Native-HR    : ${INCLUDE_NATIVE_HR}"
echo "  Save latents : ${SAVE_LATENTS} (${LATENT_SAVE_DTYPE})"
echo "  Primary Lambda: ${PRIMARY_LAMBDA}"
echo "  Log File     : ${LOG_FILE}"

printf -v RUN_COMMAND \
  'env PROJECT_ROOT=%q TOTAL_PROMPTS=%q EXPECTED_TOTAL_PROMPTS=%q PROMPT_OFFSET=%q GPU_IDS=%q SEEDS=%q OUT_ROOT=%q CLEAN_VIDEOS=%q INCLUDE_NATIVE_HR=%q SAVE_LATENTS=%q LATENT_SAVE_DTYPE=%q DRY_RUN=%q PRIMARY_LAMBDA=%q bash %q 2>&1 | tee -a %q' \
  "${PROJECT_ROOT}" "${TOTAL_PROMPTS}" "${EXPECTED_TOTAL_PROMPTS}" "${PROMPT_OFFSET}" "${GPU_IDS}" \
  "${SEEDS}" "${OUT_ROOT}" "${CLEAN_VIDEOS}" "${INCLUDE_NATIVE_HR}" "${SAVE_LATENTS}" "${LATENT_SAVE_DTYPE}" "${DRY_RUN}" \
