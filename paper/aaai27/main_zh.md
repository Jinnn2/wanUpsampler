...
TAA 的下游作用在单项指标上比聚合 VBench-5 更清晰。在固定 CLL 时，TAA 在第 40 步、第 45 步和 4 步模型上均提高成像质量，增量分别为 +0.00599、+0.00254 和 +0.00479；这些收益被美学质量与一致性指标的变化部分抵消，因而对应的 VBench-5 聚合变化仅为 +0.00087、-0.00050 和 +0.00010。结合表 3 可见，TAA 更适合被解释为选择性补偿分辨率切换所造成的成像质量与局部细节损失，而非均匀提高所有自动质量指标。
...
### 5.5 人工盲评验证细节恢复及其时序权衡

人工评价进一步检验 VBench 所显示的选择性成像质量恢复能否被感知。在 TALH-E 的相同 CLL 下，TAA 在细节和整体质量上分别取得 9/0/1 与 6/0/4 的胜/负/平，而时序稳定性以 0/8/2 偏向未对齐状态。TALH-Q 呈现相同方向：细节和整体质量分别为 8/2/0 与 6/2/2，时序稳定性为 1/9/0。评价者由此能够感知 TAA 带来的更丰富局部结构，并经常在整体渲染上偏好 TAA；与此同时，新增高频细节也引入了可测的时序代价。该成对感知证据补充了近乎不变的聚合 VBench-5，因为五项平均会稀释集中于成像细节的改善。
...
CLL 仍是广泛感知质量改善的主要来源：在第 45 步切换上，CLL-only Handoff 与完整 TALH-E 相对 Trilinear Handoff，在伪影、细节、整体质量和结构/身份保持方面均获得 10/10 的提示级胜率。TAA 则在 CLL 之上提供范围更窄、但能够被感知的局部细节精修。这一分工与 VBench-5 的因子分析结果一致；评价者一致性统计见补充材料。
...
TALH 将与时间步和调度器相关的轨迹误差交由 TAA 处理，在稳定的纯净潜变量域中学习 CLL，并通过 HTR 调用高分辨率扩散先验生成剩余高频成分。该分工建立了从模块目标到端到端生成的可解释路径。终点误差、三个工作点上一致为正的成像质量增量以及人评细节偏好共同表明，TAA 能够选择性恢复局部空间细节；但五项聚合指标包含多个基本不变的维度，新增高频细节也伴随美学与时序权衡，因此该收益不会线性转化为 VBench-5 增长。TAA 的最终作用还受到 CLL 输入分布和高分辨率后缀长度的影响，因而 2×2 因子实验与成对人评对于分析模块交互均不可缺少。
