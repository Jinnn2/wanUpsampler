\hypertarget{trajectory-aligned-latent-handoff-for-efficient-high-resolution-video-diffusion}{%
\section{Trajectory-Aligned Latent Handoff for Efficient High-Resolution Video Diffusion}\label{trajectory-aligned-latent-handoff-for-efficient-high-resolution-video-diffusion}}
...
Driven by these insights, we present \textbf{TALH} (Trajectory-Aligned Latent Handoff), which decomposes dynamic resolution handoff into three modular components: TAA, CLL, and HTR. Activated exclusively at the designated handoff step, the \textbf{Trajectory Alignment Adapter (TAA)} uses LoRA to reduce the discrepancy between the current clean prediction and the complete low-resolution trajectory endpoint. The \textbf{Clean Latent Lifter (CLL)} then learns a cross-scale mapping between separately encoded low- and high-resolution clean latents. Finally, \textbf{High-Resolution Trajectory Re-entry (HTR)} re-noises the output according to the target-resolution schedule, allowing the frozen generator to complete the high-resolution suffix. Both TAA and CLL are supervised by the \emph{same} frozen generative model: TAA uses the full low-resolution trajectory to construct trajectory alignment pairs, while CLL uses model-generated high-resolution videos and their downsampled counterparts to construct cross-resolution lifting pairs. This design yields a unified, model-endogenous self-distillation framework. Figure 1 summarizes the real-frame behavior and mixed-resolution inference path, while Figure 2 details the handoff modules and their dual model-endogenous supervision.

\includegraphics{figures/fig_teaser.png}

\textbf{Figure 1: Real-frame comparison and acceleration mechanism of TALH.} (a) Full frames and identical Crop A regions compare the content-aligned \texttt{Native-HR\ (estimated)} trajectory with Trilinear, CLL-only, and TALH-Q outputs for prompt 05, seed 9705, frame 41. The estimated trajectory is a complete 368p sampling path; the handoff outputs are 720p. (b) Native-HR Sampling executes all 50 denoising evaluations at high resolution, whereas TALH-Q and TALH-E move the first 40 and 45 evaluations to the low-resolution latent space and retain 10 and 5 high-resolution suffix evaluations, respectively.
...
\hypertarget{latent-video-generation-and-cascaded-upscaling}{%
\subsubsection{2.1 Latent Video Generation and Cascaded Upscaling}\label{latent-video-generation-and-cascaded-upscaling}}
...
\hypertarget{video-super-resolution-and-temporal-consistency}{%
\subsubsection{2.2 Video Super-Resolution and Temporal Consistency}\label{video-super-resolution-and-temporal-consistency}}
...
\hypertarget{mixed-resolution-diffusion-inference}{%
\subsubsection{2.3 Mixed-Resolution Diffusion Inference}\label{mixed-resolution-diffusion-inference}}
...
\hypertarget{few-step-distillation-and-on-policy-trajectory-learning}{%
\subsubsection{2.4 Few-Step Distillation and On-Policy Trajectory Learning}\label{few-step-distillation-and-on-policy-trajectory-learning}}
...
\hypertarget{problem-definition-and-overall-framework}{%
\subsubsection{3.1 Problem Definition and Overall Framework}\label{problem-definition-and-overall-framework}}
...
{[} x\_s\^{}L \xrightarrow{\mathrm{TAA}:v_{\phi+\Delta\theta_s}} \widetilde z\_s\^{}L \xrightarrow{U_\psi^{\mathrm{CLL}}} \widehat z\_s\^{}H \xrightarrow{\mathrm{HTR}} x\_\{s+1\}\^{}H. {]}
...
{[} E\_\{\mathrm{handoff}\}(s) \approx E\_\{\mathrm{traj}\}(s) + E\_\{\mathrm{lift}\}(s) + E\_\{\mathrm{refine}\}(s). {]}
...
\includegraphics{figures/fig_talh_overview.png}

\textbf{Figure 2: Hybrid-resolution inference and model-endogenous training in TALH.} TALH performs most denoising evaluations in the low-resolution latent space, then applies TAA trajectory alignment, CLL cross-resolution clean-latent lifting, and HTR high-resolution trajectory re-entry. TALH-Q and TALH-E retain 10 and 5 high-resolution suffix evaluations, respectively. Both the TAA alignment pairs and CLL lifting pairs are generated by the same frozen Wan model, without external paired videos, super-resolution weights, or an additional teacher.

\hypertarget{model-endogenous-dual-supervision-data}{%
\subsubsection{3.2 Model-Endogenous Dual-Supervision Data}\label{model-endogenous-dual-supervision-data}}
...
\textbf{Cross-Resolution Lifting Pairs.} We run the frozen Wan model using a set of predefined text prompts and fixed random seeds to generate high-resolution reference videos. These videos are spatially downsampled in the RGB pixel space. Both the high- and low-resolution variants are then encoded using the same Wan VAE to produce perfectly aligned cross-resolution clean latent pairs \((z_0^L, z_0^H)\). This construction preserves the frame count, semantic content, and motion trajectories, forcing CLL to learn cross-scale correspondences tightly bound to the target model's specific VAE and generative distribution.

\textbf{Trajectory Alignment Pairs.} Under identical text prompts, random seeds, initial noise, and schedulers, we roll out the complete low-resolution trajectory of the frozen model. We cache the intermediate low-resolution state \(x_s^L\) immediately preceding the handoff step and utilize the final low-resolution trajectory endpoint \(z_T^L\) as the ground-truth target. Consequently, CLL receives cross-resolution structural supervision, while TAA receives cross-timestep trajectory supervision, forming a unified model-endogenous resolution-trajectory self-distillation framework.
...
\hypertarget{cross-resolution-clean-latent-lifting}{%
\subsubsection{3.3 Cross-Resolution Clean Latent Lifting}\label{cross-resolution-clean-latent-lifting}}
...
{[} U\_\psi\textsuperscript{\{\mathrm{CLL}\}:\mathbb{R}}\{B\times16\times F\times h\times w\} \rightarrow \mathbb{R}\^{}\{B\times16\times F\times H\times W\}. {]}
...
{[} \mathcal L\_\{\mathrm{CLL}\}=\mathcal L\_\{\mathrm{rec}\} +\lambda\emph{\{\mathrm{content}\}\mathcal L}\{\mathrm{content}\} +\lambda\emph{\{\mathrm{temp}\}\mathcal L}\{\mathrm{temp}\}. {]}

where

{[} \mathcal L\_\{\mathrm{rec}\} =\mathbb E\left[\sqrt{(\widehat z_0^H-z_0^H)^2+\epsilon^2}\right]. {]}

{[} \mathcal L\_\{\mathrm{content}\} =\lVert D(\widehat z\_0\textsuperscript{H)-z\_0}L\rVert\emph{1, \qquad \mathcal L}\{\mathrm{temp}\} =\lVert\Delta\_F\widehat z\_0\^{}H-\Delta\_Fz\_0\^{}H\rVert\_1. {]}
...
\hypertarget{handoff-local-trajectory-alignment}{%
\subsubsection{3.4 Handoff-Local Trajectory Alignment}\label{handoff-local-trajectory-alignment}}
...
{[} \widetilde z\_s\^{}L =x\_s\^{}L-\sigma\emph{s v}\{\phi+\Delta\theta\_s\}(x\_s\^{}L,s,c). {]}
...
{[} \mathcal L\_\{\mathrm{align}\} =\lVert\widetilde z\_s\textsuperscript{L-z\_T}L\rVert\_1 +0.1\lVert\widetilde z\_s\textsuperscript{L-z\_T}L\rVert\_2\^{}2. {]}
...
\hypertarget{cached-prefix-state-consistency}{%
\subsubsection{3.5 Cached-Prefix State Consistency}\label{cached-prefix-state-consistency}}
...
{[} x\_k\textsuperscript{\{\mathrm{adapted}\}=x\_k}\{\mathrm{base}\}, \qquad \forall k\leq s. {]}
...
\hypertarget{high-resolution-trajectory-re-entry-and-two-instances}{%
\subsubsection{3.6 High-Resolution Trajectory Re-entry and Two Instances}\label{high-resolution-trajectory-re-entry-and-two-instances}}

Once TAA produces the aligned low-resolution clean latent, CLL outputs the upscaled high-resolution clean latent:

{[} \widehat z\_s\textsuperscript{H=U\_\psi}\{\mathrm{CLL}\}(\widetilde z\_s\^{}L). {]}
...
{[} x\_\{s+1\}\^{}H =(1-\sigma\emph{\{s+1\})\widehat z\_s\^{}H +\sigma}\{s+1\}\epsilon\^{}H. {]}

The remaining denoising calculations from step \(s+1\) to \(T\) are subsequently completed by the frozen Wan denoiser operating on the high-resolution latent space. Unlike an \emph{Interpolated-Flow Re-entry} strategy that directly interpolates low-resolution velocity flows into the high-resolution space, TALH utilizes \emph{Target-Resolution Noise Re-entry}. This design ensures that newly introduced spatial frequencies are synthesized natively by the high-resolution generative priors of the base model.

Our \textbf{50-step pipeline} uses Wan2.1 T2V-1.3B, configured with 50 inference steps, a sample shift of 8, and a CFG scale of 6. Extensive handoff-step scanning identified two representative operational points. The quality-centric \textbf{TALH-Q} (TALH@40) reserves 10 high-resolution steps, granting a larger budget for texture generation while managing a wider residual trajectory gap that requires substantial TAA alignment. Conversely, the efficiency-centric \textbf{TALH-E} (TALH@45) reserves only 5 high-resolution steps to minimize latency; its handoff state sits closer to a fully converged clean latent, resulting in more stable CLL inputs and narrower TAA corrections. Together, these two instances define the quality-efficiency Pareto frontier of TALH.

Our \textbf{4-step pipeline} leverages Wan2.1 T2V-14B StepDistill-CfgDistill, operating on four nominal timesteps \([1000, 750, 500, 250]\) with a sample shift of 5. The distilled instance \textbf{TALH-D4} (TALH@3-of-4) runs low-resolution sampling for the first two steps. At step 3, TAA predicts the aligned low-resolution clean latent, which is scaled by CLL and injected back into the high-resolution trajectory via HTR at step 4. A single final high-resolution denoising step completes the generation.

\hypertarget{experiments}{%
\subsection{4 Experiments}\label{experiments}}

\hypertarget{experimental-objectives}{%
\subsubsection{4.1 Experimental Objectives}\label{experimental-objectives}}
...
\hypertarget{data-and-training-setup}{%
\subsubsection{4.2 Data and Training Setup}\label{data-and-training-setup}}
...
\hypertarget{baselines-and-factorial-experiments}{%
\subsubsection{4.3 Baselines and Factorial Experiments}\label{baselines-and-factorial-experiments}}
...
\tightlist
\item
  \textbf{Native-HR Sampling:} Standard high-resolution inference using the original model, where all steps are executed natively on the \(720\text{p}\) latent space.
\item
  \textbf{Native-LR Sampling:} Standard low-resolution inference across all steps followed by direct VAE decoding.
\item
  \textbf{Trilinear Handoff:} Mixed-resolution sampling without trajectory alignment, using LightX2V-style trilinear upscaling at the handoff step.
\item
  \textbf{CLL-only Handoff:} Trajectory alignment is disabled, replacing trilinear interpolation solely with our CLL module.
\item
  \textbf{TAA + Trilinear Handoff:} Engages TAA for trajectory alignment but falls back to trilinear interpolation for upscaling.
\item
  \textbf{TALH:} Our full framework combining the TAA-aligned trajectory state with the CLL resolution operator.
\item
  \textbf{Oracle-Aligned CLL:} Bypasses TAA by directly feeding the true low-resolution trajectory endpoint into CLL, serving as the theoretical upper bound for trajectory alignment.
\item
  \textbf{Joint Trajectory-Scale Lifting (JTSL):} A baseline model trained to directly map the low-resolution intermediate prediction at the handoff step to the high-resolution target latent in a single step.
\item
  \textbf{Endpoint Re-entry Baseline:} Runs the low-resolution trajectory to completion, applies CLL upscaling, and performs a single low-intensity high-resolution refinement step via HTR.
\end{enumerate}
...
\hypertarget{metrics-and-fairness}{%
\subsubsection{4.4 Metrics and Fairness}\label{metrics-and-fairness}}
...
The comprehensive VBench score (\textbf{VBench-5}) is defined as the unweighted mean of five core dimensions: \emph{subject consistency}, \emph{background consistency}, \emph{motion smoothness}, \emph{aesthetic quality}, and \emph{imaging quality}. To guarantee strict equity, all pairwise configurations share identical text prompts, random seeds, initial noise, schedulers, and guidance parameters. Prompt-level score deltas are evaluated via 10,000 paired bootstrap iterations to compute \(95\%\) confidence intervals, and two-sided exact sign tests measure win-loss significance.
...
\hypertarget{core-comparison-quality-efficiency-pareto-frontier-against-native-hr-sampling}{%
\subsubsection{5.1 Core Comparison: Quality-Efficiency Pareto Frontier Against Native-HR Sampling}\label{core-comparison-quality-efficiency-pareto-frontier-against-native-hr-sampling}}
...
\includegraphics{figures/fig_quality_efficiency.png}

\textbf{Figure 3: Quality-efficiency trade-off relative to Native-HR Sampling.} (a) TALH-Q and TALH-E reduce end-to-end latency to 138.36 and 114.26 seconds, providing quality- and efficiency-oriented operating points; Endpoint Re-entry is faster but incurs a larger aggregate quality decrease. (b) Absolute per-dimension VBench changes of TALH-Q and TALH-E relative to Native-HR Sampling.
...
\midrule
\endhead
Native-HR Sampling & 0/50 & 253.10 ± 1.53 & --- & 1.00× & \textbf{0.82836} & 100.00\% \\
TALH-Q (TALH@40) & 40/10 & 138.36 ± 0.50 & 45.33\% & 1.83× & 0.80983 & 97.76\% \\
TALH-E (TALH@45) & 45/5 & \textbf{114.26 ± 1.52} & \textbf{54.85\%} & \textbf{2.22×} & 0.80792 & 97.53\% \\
\bottomrule
\end{longtable}

\textbf{Table 1: Core system comparison of TALH against Native-HR Sampling.} Timings are averaged over 5 independent cold-start runs. VBench-5 scores are computed across 10 matched text prompts. TAA, CLL, HTR, VAE decoding, and dynamic model loading overheads are fully included.
...
\hypertarget{clean-latent-lifting-outperforms-fixed-interpolation}{%
\subsubsection{5.2 Clean Latent Lifting Outperforms Fixed Interpolation}\label{clean-latent-lifting-outperforms-fixed-interpolation}}
...
\textbf{Table 2: Operator-level comparison between CLL and trilinear upscaling (\(n=50\)).} Each row is benchmarked against the identical high-resolution VAE-encoded latent as ground truth.

\hypertarget{trajectory-alignment-narrows-residual-endpoint-gaps}{%
\subsubsection{5.3 Trajectory Alignment Narrows Residual Endpoint Gaps}\label{trajectory-alignment-narrows-residual-endpoint-gaps}}
...
\textbf{Table 3: Paired trajectory alignment performance of TAA.} The first two rows evaluate native \(368\text{p}\) low-resolution trajectories; the third row evaluates an independent transfer test set on the 4-step distilled model.
...
\midrule
\endhead
Wan50 / @40 (TALH-Q) & 0.77812 & 0.77901 & 0.80896 & \textbf{0.80983} & +0.03085 & +0.00087 & +0.03171 \\
Wan50 / @45 (TALH-E) & 0.76776 & 0.76734 & \textbf{0.80842} & 0.80792 & +0.04066 & -0.00050 & +0.04016 \\
Distill4 / @3-of-4 (TALH-D4) & 0.81796 & 0.81909 & 0.85670 & \textbf{0.85680} & +0.03874 & +0.00010 & +0.03884 \\
\bottomrule
\end{longtable}

\textbf{Table 4: Comprehensive VBench-5 generation scores (\(n=10\)).} \texttt{TAA\ Gain\ under\ CLL} isolates the marginal increment of engaging TAA while keeping CLL fixed.
...
\midrule
\endhead
TAA vs.~Unaligned, both using CLL (@40) & 8/2/0 & 6/2/2 & 1/9/0 \\
TAA vs.~Unaligned, both using CLL (@45) & \textbf{9/0/1} & \textbf{6/0/4} & 0/8/2 \\
CLL-only vs.~Trilinear Handoff (@45) & \textbf{10/0/0} & \textbf{10/0/0} & \textbf{8/0/2} \\
TALH-E vs.~Trilinear Handoff (@45) & \textbf{10/0/0} & \textbf{10/0/0} & \textbf{9/0/1} \\
\bottomrule
\end{longtable}

\textbf{Table 5: Prompt-level majority votes from the human study (\(n=10\), 3 raters per prompt).} Bold entries denote statistical significance via a two-sided exact sign test (\(p < 0.05\)).
...
\hypertarget{evaluation-against-strong-speed-baselines-and-handoff-planning}{%
\subsubsection{5.6 Evaluation Against Strong Speed Baselines and Handoff Planning}\label{evaluation-against-strong-speed-baselines-and-handoff-planning}}
...
\hypertarget{transferability-to-distilled-samplers}{%
\subsubsection{5.7 Transferability to Distilled Samplers}\label{transferability-to-distilled-samplers}}
...
\hypertarget{qualitative-comparison}{%
\subsubsection{5.8 Qualitative Comparison}\label{qualitative-comparison}}
...
References are managed uniformly via \texttt{references.bib} and are rendered according to the AAAI style guide in the final LaTeX document.
