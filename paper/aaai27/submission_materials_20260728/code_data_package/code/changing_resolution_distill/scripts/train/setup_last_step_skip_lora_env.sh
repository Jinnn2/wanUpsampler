...
clone_or_update_diffsynth() {
  if [[ ! -d "${DIFFSYNTH_REPO}/.git" ]]; then
    if [[ -z "${DIFFSYNTH_GIT_URL}" ]]; then
      echo "Provide an existing DIFFSYNTH_REPO checkout or set DIFFSYNTH_GIT_URL." >&2
      exit 1
    fi
