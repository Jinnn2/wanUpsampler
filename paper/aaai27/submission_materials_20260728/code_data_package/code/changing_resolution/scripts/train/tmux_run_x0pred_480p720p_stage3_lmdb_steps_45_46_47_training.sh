...
WORKER_LOG_DIR="${WORKER_LOG_DIR:-${PROJECT_ROOT}/logs/changing_resolution_stage3_x0pred_lmdb_steps_${STEP_TAG}_train}"
...
if (( failed != 0 )); then
  echo "At least one Stage 3 training worker failed. Check logs under: ${WORKER_LOG_DIR}" >&2
  exit 1
fi
