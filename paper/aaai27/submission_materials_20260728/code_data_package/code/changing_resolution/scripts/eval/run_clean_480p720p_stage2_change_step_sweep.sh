...
# Stage 2 short runs can have a lagging EMA; default to raw weights for eval.
USE_EMA="${USE_EMA:-0}"
...
mkdir -p "${OUT_DIR}"/{configs,stop480,interp720,stage2_720,compare}
...
export PROJECT_ROOT CHECKPOINT TRAIN_CONFIG RATE INFER_STEPS NUM_FRAMES GUIDE_SCALE SAMPLE_SHIFT BRIDGE_USE_EMA HR_H HR_W LR_H LR_W STAGE2_RESIDUAL_SKIP
...
for prompt in "${prompts[@]}"; do
  ...
  for change_step in "${steps[@]}"; do
    ...
    run_infer "wan2.1_clean_resizer_bridge" "${cfg_stage2}" "${prompt}" "${seed}" "${stage2_720}"
    ...
    make_labeled_panel "${stage2_720}" "${p_stage2}" "stage2 720 step ${change_step}->${INFER_STEPS}"
    ffmpeg -hide_banner -loglevel error -y \
      -i "${p_stop}" -i "${p_interp}" -i "${p_stage2}" \
...
echo "Stage 2 change-step sweep videos ready: ${OUT_DIR}/compare"
